...
//	extended ::= ( alphanum | â_â | â-â | â.â )+
//	quoted-string ::= â"â ( ~["\#x0D#x0A#x00] | â\"â|â\\â)* â"â
 //	argument ::= extended | quoted-string
 //	parameter ::= directive | attribute
//	directive ::= extended â:=â argument
//	attribute ::= extended â=â argument
//	path ::= path-unquoted | (â"â path-unquoted â"â)
 //	path-unquoted ::= path-sep | path-sep? path-element (path-sep path-element)*
 //	path-element ::= ~[/"\#x0D#x0A#x00]+
//	path-sep ::= â/â
//	header ::= clause ( â,â clause ) *
//	clause ::= path ( â;â path ) * ( â;â parameter ) *
	...
	public static final String PATH = "(?:" + PATH_UNQUOTED_NT + ")|(?:\"" + PATH_UNQUOTED + "\")";
 	 ...
 	 * unique-name ::= identifier ( â.â identifier )*
