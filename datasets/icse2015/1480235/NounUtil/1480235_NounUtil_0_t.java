 ...
 public class NounUtil {
   ...
   static {
     String[] strs = new String[]{"ë±", "ë¤","ì","ê°","ë¿","ë³"};
     for(String str:strs) {
       DNouns.add(str);
     }
   };
   ...
   public static boolean analysisMJ(AnalysisOutput o, List candidates) throws MorphException {
     ...
     if(o.getStem().charAt(strlen-1)!='ê¸°'&&!(chrs.length==3&&chrs[2]=='ã')) return false;
 
     String start = o.getStem();
     String end = "";
     if(o.getStem().charAt(strlen-1)=='ê¸°') {
       start = o.getStem().substring(0,strlen-1);
       end = "ê¸°";
     }else if(o.getStem().charAt(strlen-1)=='ì') {
       start = o.getStem().substring(0,strlen-1);
       end = "ì";
     }
 ...
 //    }
   ...
   /**
    * ë³µí©ëªì¬ìì ë¨ìëªì¬ë¥¼ ë¶ë¦¬í´ë¸ë¤.
    * ë¦¬ì¤í¸ì ê°ì¥ ë§ì§ë§ì ìì¹í ë¨ì´ê° ìµì¥ë¨ì´ì´ë¤.
    * @param str  ë³µí©ëªì¬
    * @param pos
    * @param o    ë¶ìê²°ê³¼
   * return    ë¨ìëªì¬ ë¦¬ì¤í¸
    * @throws MorphException
    */
   ...
   /*
      * ë§ì§ë§ ìì ì´ ëªì¬í ì ë¯¸ì¬(ë±,ì..)ì¸ì§ ì¡°ì¬íë¤.
      */
