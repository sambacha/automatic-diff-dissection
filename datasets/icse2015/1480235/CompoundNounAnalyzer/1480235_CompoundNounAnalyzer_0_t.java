 ...
 public class CompoundNounAnalyzer {
   ...
   private void analyzeLongText(String input,List<CompoundEntry> outputs, boolean isFirst) throws MorphException {
     ...
     if(input.length()>20) return; // 20ê¸ì ì´ìì ë³µí©ëªì¬ë ë¬´ìí¨.
 ...
 //        if(ftry&&str.length()==2&&DictionaryUtil.existSuffix(suffix)) return null; // 2ê¸ì+ì ë¯¸ì´ ê²°í©ì´ ë§ì¼ë¯ë¡  
 //        return str;
 //      }
 //      
 //      return null;
 ...
 //  }
   ...
   private CompoundEntry[] analysisBySplited(int[] units, String input, boolean isFirst) throws MorphException {
     ...
     for(int i=0;i<units.length;i++) {
       ...
       entries[i] = analyzeSingle(str); // CompoundEntry ë¡ ë³í
   ...
   /**
    * ìë ¥ë String ì CompoundEntry ë¡ ë³í
    * @param input
