   ...
   * @return candidates
   ...
   public List<AnalysisOutput> analyze(String input, int pos) throws MorphException {    
     ...
     for(AnalysisOutput o:candidates) { 
       ...
       if(o.getScore()==AnalysisOutput.SCORE_FAIL) continue; // ë¶ììë ì±ê³µíì¼ë, ì ì½ì¡°ê±´ì ì¤í¨
 ...
 //    if(candidates.size()!=0&&candidates.get(0).getScore()==AnalysisOutput.SCORE_CORRECT) return;
   ...
   /**
    * ì²´ì¸ + ì¡°ì¬ (PTN_NJ)
    * ì²´ì¸ + ì©ì¸íì ë¯¸ì¬ + 'ì/ê¸°' + ì¡°ì¬ (PTN_NSMJ
    * ì©ì¸ + 'ì/ê¸°' + ì¡°ì¬ (PTN_VMJ)
    * ì©ì¸ + 'ì/ì´' + ë³´ì¡°ì©ì¸ + 'ì/ê¸°' + ì¡°ì¬(PTN_VMXMJ)
    * 
    * @param stem
    * @param end
    * @param candidates
   ...
   public void analysisWithJosa(String stem, String end, List candidates) throws MorphException {
     ...
     if(!DictionaryUtil.existJosa(end)||
         ...
         (chrs.length==2&&(ConstraintUtil.isThreeJosa(end))||"".equals(end))) return; // ì°ê²°ì´ ê°ë¥í ì¡°ì¬ê° ìëë©´...
   ...
   /**
    * 
    *  1. ì¬ëë°ë¤ : ì²´ì¸ + ì©ì¸íì ë¯¸ì¬ + ì´ë¯¸ (PTN_NSM) <br>
    *  2. ì¬ëë°ìë³´ë¤ : ì²´ì¸ + ì©ì¸íì ë¯¸ì¬ + 'ì/ì´' + ë³´ì¡°ì©ì¸ + ì´ë¯¸ (PTN_NSMXM) <br>
    *  3. íêµììì´ë¤ : ì²´ì¸ + 'ìì/ë¶í°/ììë¶í°' + 'ì´' + ì´ë¯¸ (PTN_NJCM) <br>
    *  4. ëë¤ : ì©ì¸ + ì´ë¯¸ (PTN_VM) <br>
    *  5. ëìì´ë¤ : ì©ì¸ + 'ì/ê¸°' + 'ì´' + ì´ë¯¸ (PTN_VMCM) <br>
    *  6. ëìì£¼ë¤ : ì©ì¸ + 'ì/ì´' + ë³´ì¡°ì©ì¸ + ì´ë¯¸ (PTN_VMXM) <br>
    *  
    * @param stem
    * @param end
    * @param candidates
   ...
   public void analysisWithEomi(String stem, String end, List candidates) throws MorphException {
     ...
     if(morphs[0]==null) return; // ì´ë¯¸ê° ì¬ì ì ë±ë¡ëì´ ìì§ ìë¤ë©´....
     ...
     try {    
       ...
       if(entry!=null&&!("ì".equals(end)&&entry.getFeature(WordEntry.IDX_REGURA)==IrregularUtil.IRR_TYPE_LIUL)) {              
         ...
         if(features[SyllableUtil.IDX_YNPLN]=='0'||morphs[1].charAt(0)!='ã´')   return;
       }
 
       String[] irrs = IrregularUtil.restoreIrregularVerb(o.getStem(), o.getPomi()==null?o.getEomi():o.getPomi());
 
       if(irrs!=null) { // ë¶ê·ì¹ëì¬ì¸ ê²½ì°
         AnalysisOutput output = o.clone();
 ...
 //        if(entry!=null && VerbUtil.constraintVerb(o.getStem(), o.getPomi()==null?o.getEomi():o.getPomi())) { // 4. ëë¤ (PTN_VM)
         output.setScore(AnalysisOutput.SCORE_CORRECT);
         MorphUtil.buildPtnVM(output, candidates);
 //        }        
       }
   ...
   /**
    * ë³µí©ëªì¬ì¸ì§ ì¡°ì¬íê³ , ë³µí©ëªì¬ì´ë©´ ë¨ìëªì¬ë¤ì ì°¾ëë¤.
    * ë³µí©ëªì¬ì¸ì§ ì¬ë¶ë ë¨ìëªì¬ê° ëª¨ë ì¬ì ì ìëì§ ì¬ë¶ë¡ íë¨íë¤.
    * ë¨ìëªì¬ë 2ê¸ì ì´ì ë¨ì´ììë§ ì°¾ëë¤.
    * @throws MorphException
    */
 ...
 //       for(CompoundEntry c :results) System.out.println(c.getWord()+":"+c.isExist());
   ...
   private boolean constraint(AnalysisOutput o) throws MorphException  {
     ...
     if("íí´".equals(cnouns.get(cnouns.size()-1).getWord())) {
       ...
       if("ë´".equals(o.getVsfx())&&cnouns.get(cnouns.size()-1).getWord().length()!=1) {
         ...
         // ì§ì¬ëíë¤ ì ê°ì ê²½ì°ì ë·ê¸ìê° 1ê¸ìì´ë©´ ì ì¸
         return false;
       }         
     }       
     return true;
