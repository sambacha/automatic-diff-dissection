 ...
 public class TestJapaneseTokenizer extends BaseTokenStreamTestCase {
   ...
   public void testDecomposition5() throws Exception {
    try (TokenStream ts = analyzer.tokenStream("bogus", "ãããããããããããããããããããããããããããããããããããããããã")) {
     ts.reset();
   ...
   */
   ...
   public void testTwoSentences() throws Exception {
     /*
     //TokenStream ts = a.tokenStream("foo", "å¦¹ã®å²å­ã§ããä¿ºã¨å¹´å­ã§ãä»åé¨çã§ãã");
   ...
   public void testOnlyPunctuation() throws IOException {
    try (TokenStream ts = analyzerNoPunct.tokenStream("foo", "ãããã")) {
     ts.reset();
   ...
   public void testEnd() throws Exception {
     assertTokenStreamContents(analyzerNoPunct.tokenStream("foo", "ããã¯æ¬ã§ã¯ãªã"),
         new String[] { "ãã", "ã¯", "æ¬", "ã§", "ã¯", "ãªã" },
         new int[] { 0, 2, 3, 4, 5, 6 },
         new int[] { 2, 3, 4, 5, 6, 8 },
         new Integer(8)
   ...
   public void testUserDict() throws Exception {
     ...
     assertTokenStreamContents(analyzer.tokenStream("foo", "é¢è¥¿å½éç©ºæ¸¯ã«è¡ã£ã"),
   ...
   public void testUserDict2() throws Exception {
     ...
     assertTokenStreamContents(analyzer.tokenStream("foo", "æéé¾"),
                               new String[] { "æéé¾"  },
                               new int[] { 0 },
                               new int[] { 3 },
                               new Integer(3)
   ...
   */
   ...
   public void testWithPunctuation() throws Exception {
     assertAnalyzesTo(analyzerNoPunct, "ç¾½ç°ãç©ºæ¸¯",
                      new String[] { "ç¾½ç°", "ç©ºæ¸¯" },
                      new int[] { 1, 1 });
   }
 
   public void testCompoundOverPunctuation() throws Exception {
     assertAnalyzesToPositions(analyzerNoPunct, "dÎµÎµÏ¶Ï¢ÏÎÏ·ÎÍºç¾½ç°",
                               new String[] { "d", "Îµ", "Îµ", "Ï¢ÏÎÏ·ÎÍº", "ç¾½ç°" },
                               new int[] { 1, 1, 1, 1, 1},
                               new int[] { 1, 1, 1, 1, 1});
   }
 }
